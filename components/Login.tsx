...
const Login: React.FC<LoginProps> = ({ onLoginSuccess, isSsoEnabled }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onLoginSuccess(user);
    // FIX: Added opening curly brace to fix syntax error in catch block.
    } catch (err: any) {
  ...
  const handleSsoLogin = () => {
    // This is the first step of a real SAML flow.
    // It redirects the user to the configured Identity Provider (IdP).
    const ssoUrl = localStorage.getItem('ssoUrl');

    if (ssoUrl) {
        // In a real application, the backend would need to handle the SAML callback from the IdP.
        // This redirect initiates that process.
        window.location.href = ssoUrl;
    } else {
        alert("O Login SSO está habilitado, mas a URL do SSO não foi configurada pelo administrador nos Ajustes de Integração. Por favor, contate o suporte.");
    }
  };
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-gray-100 dark:bg-dark-bg p-4">
      ...
      <div className="absolute bottom-4 text-center text-gray-500 dark:text-dark-text-secondary text-xs">
        ...
        <p className="text-gray-400 dark:text-gray-500 mt-1">&copy; 2026 Dev: Mrcelo Reis</p>
      </div>
    </div>
  );
};
